  ...
  async function handleSubmit(e) {
    ...
    if (mode === "login") {
      ...
      try {
        ...
        setErrors({ _form: err.message || "Đăng nhập thất bại." });
      } finally {
        setLoading(false);
      }
      return;
    ...
    if (role === "seller") {
      ...
      try {
        ...
        setErrors({ _form: err.message || "Đăng ký thất bại." });
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleTaxContinue() {
    ...
    try {
      const res = await apiRegister({
        ...
        password: form.password,
        role: "seller",
        cooperative_name: form.bizName.trim() || null,
      });
      onLogin?.(res.user);
      onClose?.();
    } catch (err) {
      // Quay lại form để hiển thị lỗi (vd: SĐT đã đăng ký)
      setStep(STEPS.FORM);
      setErrors({ _form: err.message || "Đăng ký thất bại." });
    } finally {
      setLoading(false);
      ...
      <div className="lm-card">
        ...
        {(mode === "login" || (mode === "register" && step === STEPS.FORM)) && (
          <form className="lm-form" onSubmit={handleSubmit}>
            ...
            {errors._form && (
              <p className="lm-error" style={{ textAlign: "center", marginBottom: 8 }}>
                {errors._form}
              </p>
            )}
